...
use WprAddons\Classes\Modules\WPR_Woo_Grid_Helpers;
...
class Wpr_Woo_Grid extends Widget_Base {
	...
	public function add_option_element_select() {
		return [
			...
			'read-more' => esc_html__( 'View More', 'wpr-addons' )
		];
	}
		...
		// All Filters
			...
			foreach ( $all_filters as $key => $filter ) {
				...
				if ( 0 === $filter->parent ) {
					$children = get_term_children( $filter->term_id, $taxonomy );
					$data_role = ! empty($children) ? ' data-role="parent"' : '';
					$hidden_class = WPR_Woo_Grid_Helpers::get_hidden_filter_class($filter->slug, $settings);
	...
	public function add_grid_settings( $settings, $settings_new ) {
		...
		$layout_settings = [
			...
			'pagination_max_pages' => WPR_Woo_Grid_Helpers::get_max_num_pages( $settings )
		];
	...
	protected function render() {
		...
		while ( $posts->have_posts() ) : $posts->the_post();
			...
			// Content: Above Media
			WPR_Woo_Grid_Helpers::get_elements_by_location( 'above', $settings, get_the_ID(), $settings_new );
			...
			echo '<div class="wpr-grid-media-wrap'. esc_attr(WPR_Woo_Grid_Helpers::get_image_effect_class( $settings )) .' " data-overlay-link="'. esc_attr( $settings['overlay_post_link'] ) .'">';
				...
				echo '<div class="wpr-grid-media-hover wpr-animation-wrap">';
					...
					// Content: Over Media
					WPR_Woo_Grid_Helpers::get_elements_by_location( 'over', $settings, get_the_ID(), $settings_new );

				echo '</div>';
			echo '</div>';

			// Content: Below Media
			WPR_Woo_Grid_Helpers::get_elements_by_location( 'below', $settings, get_the_ID(), $settings_new );
		...
		// Pagination
		WPR_Woo_Grid_Helpers::render_grid_pagination( $settings );
