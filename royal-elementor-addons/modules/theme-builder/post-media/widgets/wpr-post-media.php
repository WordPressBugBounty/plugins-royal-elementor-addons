...
class Wpr_Post_Media extends Widget_Base {
	...
	protected function register_controls() {
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} .wpr-featured-media-image',
				'separator' => 'before'
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .wpr-featured-media-image' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
					'{{WRAPPER}} .wpr-featured-media-image img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	public function render_post_gallery( $settings ) {
		...
		$slider_options = [
			...
			'speed' => absint( ( floatval( $settings['gallery_slider_effect_duration'] ?: 1 ) ) * 1000 ),
			'arrows' => true,
			'dots' => true,
			'autoplay' => ( $settings['gallery_slider_autoplay'] === 'yes' ),
			'autoplaySpeed' => absint( ( floatval( $settings['gallery_slider_autoplay_duration'] ?: 1 ) ) * 1000 ),
